...
| <a name="input_nat_endpoint_independent_mapping"></a> [nat\_endpoint\_independent\_mapping](#input\_nat\_endpoint\_independent\_mapping) | Enable endpoint-independent mapping on each Cloud NAT so the same internal source maps to the same external port regardless of destination (lets NAT-traversal protocols such as WireGuard/Tailscale establish direct paths). Incompatible with dynamic port allocation. | `bool` | `false` | no |
